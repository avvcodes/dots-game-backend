...
app.get("/", (_req, res) => {
  res.json({
    status: "ok",
    service: "Dots & Boxes Socket.IO server",
  });
});
...
io.on("connection", (socket) => {
  ...
  socket.on("room:create", ({ name }, callback) => {
    ...
    const room = {
      ...
      players: {
        p1: {
          id: socket.id,
          name: (name || "Player 1").trim().slice(0, 16),
          connected: true,
        },
        p2: null,
      },
  ...
  socket.on("room:join", ({ code, name }, callback) => {
    ...
    room.chat.push({
      name: "System",
      text: `${room.players.p2.name} joined the room.`,
    });
  ...
  socket.on("game:move", ({ type, row, column }, callback) => {
    ...
    if (room.game[type].includes(line)) {
      return callback?.({ ok: false });
    }
    ...
    for (const [boxRow, boxColumn] of nearbyBoxes) {
      ...
      if (
        isValidBox &&
        !room.game.boxes[boxId] &&
        boxComplete(boxRow, boxColumn, room.game)
      ) {
    ...
    if (!claimedBox) {
      room.game.turn = playerId === "p1" ? "p2" : "p1";
    }

    if (
      Object.keys(room.game.boxes).length ===
